<nav class="bg-indigo-900" x-data="{ open: false }">
        ...
        <div class="relative flex items-center justify-between h-16">
            ...
            <div class="flex items-center justify-between flex-1 sm:items-stretch sm:justify-between">
                ...
                <a href="/" class="flex items-center flex-shrink-0">
                    <img class="block w-auto h-8 lg:hidden" src="{{ asset('img/logo.png') }}" alt="Workflow">
                    <img class="hidden w-auto h-8 lg:block" src="{{ asset('img/logo.png') }}">
                </a>
                <div class="hidden md:block md:ml-6">
                    <div class="flex w-full space-x-4">
                        <!-- Current: "bg-gray-900 text-white", Default: "text-gray-300 hover:bg-gray-700 hover:text-white" -->
                        <!-- <a href="#" class="px-3 py-2 text-sm font-medium text-white bg-gray-900 rounded-md">Dashboard</a> -->
                        <a href="/" class="flex justify-center px-3 py-2 text-sm font-black text-yellow-400 rounded-md hover:bg-red-700 hover:text-yellow-300"  aria-current="page">Inicio</a>
                        @foreach ($categories as $category)
                        <a href="{{route('posts.category',$category)}}" class="flex justify-center px-3 py-2 text-sm font-black text-yellow-400 rounded-md hover:bg-red-700 hover:text-yellow-300"  aria-current="page">{{$category->name}}</a>
                        @endforeach
                    </div>
    ...
    <div class="sm:hidden" x-show="open" x-on:click.away="open = false">
        <div class="px-2 pt-2 pb-3 space-y-1">
            ...
            <a href="/" class="block px-3 py-2 text-base font-medium text-white bg-gray-900 rounded-md" aria-current="page">Inicio</a>
            @foreach ($categories as $category)
            <a href="{{route('posts.category',$category)}}" class="block px-3 py-2 text-base font-medium text-white bg-gray-900 rounded-md" aria-current="page">{{$category->name}}</a>
